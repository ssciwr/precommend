...
* Python source files:
  * The Python package source files are located in the `precommit_recommendations` directory.
  * The file `precommit_recommendations.__main__.py` defines a command line interface that
    is accessible both as the command `precommit_recommendations` and via
    `python -m precommit_recommendations`.
  * `tests/test_cli.py` contains rudimentary testing of this CLI.
  * `tests/test_precommit_recommendations.py` contains the unit tests for the package.
